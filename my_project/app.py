...
if uploaded_file:
    with st.spinner('데이터를 한 칸씩 이동하여 보고서를 생성 중입니다...'):
        try:
            # 1. 인풋 파일 읽기
            ...
            if os.path.exists(template_path):
                ...
                for row_num in range(11, 17):
                    ...
                    if key_i:
                        ws[f'J{row_num}'] = s2_total_counts.get(key_i, 0) # 전체 (기존 K)
                        ws[f'K{row_num}'] = s2_out_counts.get(key_i, 0)   # 출고 (기존 L)
                        ws[f'L{row_num}'] = s2_hold_counts.get(key_i, 0)  # 보유 (기존 M)
                        ws[f'M{row_num}'] = s2_sum_values.get(key_i, 0)   # 합계 (기존 N)
